...
import { getReviewItems, GetReviewItemsParams } from "../services/reviewItemsApi";
...
export function useReviewQueue(initialStatus?: ReviewStatus) {
  ...
  const handlePriorityChange = useCallback((priorityArray: ("low" | "medium" | "high" | "critical")[]) => {
    setPriority(priorityArray);
    setPagination((prev) => ({ ...prev, page: 1 }));
  }, []);

  const handleSortingChange = useCallback((newSorting: Array<{ field: string; direction: "asc" | "desc" }>) => {
    setSorting(newSorting.length > 0 ? newSorting : [{ field: "updatedAt", direction: "desc" }]);
    setPagination((prev) => ({ ...prev, page: 1 }));
  }, []);
  ...
  const params: GetReviewItemsParams = {
    ...
    sortBy: sorting.length > 0 ? sorting.map((s) => s.field as "updatedAt" | "createdAt" | "priority" | "status") : undefined,
